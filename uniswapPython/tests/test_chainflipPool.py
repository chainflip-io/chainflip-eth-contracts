...
def test_notClearPosition_ifNoMoreLiquidity(accounts, mediumPoolInitializedAtZero):
    ...
    pool.burnLimitOrder(
        TEST_TOKENS[1], accounts[1], iniTick + pool.tickSpacing, 0
    )
    ...
    (_, _, _, amount0, amount1) = pool.burnLimitOrder(TEST_TOKENS[0], accounts[1], iniTick, expandTo18Decimals(1))
    assert amount0 > 0
    assert amount1 > 0
    (_, _, _, amount0, amount1) = pool.burnLimitOrder(
        TEST_TOKENS[1], accounts[1], iniTick + pool.tickSpacing, expandTo18Decimals(1)
...
def test_swapGaps_oneForZero(initializedPoolMedium12TickSpacing, accounts):
    ...
    pool.burnLimitOrder(
        TEST_TOKENS[0], accounts[0], 120192, 0
    )
...
def test_swapGaps_zeroForOne(initializedPoolMedium12TickSpacing, accounts):
    ...
    # Poke to get the fee amounts
    pool.burnLimitOrder(
        TEST_TOKENS[1], accounts[0], -120192, 0
    )
...
def test_precision_zeroForOne():

    print("Check precision in tick.oneMinusPercSwap")

    numberOfSwaps=1000
    st_mintAmount=1000000000000000000
    st_swapAmountsPerc=10
...
def test_precision_closeTo1(createPoolLow, accounts):
    ...
    pool.mintLimitOrder(TEST_TOKENS[1], accounts[2], 0, 14)
...
# For some reason in random testing we can't use fixtures
