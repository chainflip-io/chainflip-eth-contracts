...
def test_collectFee_currentPrice(lowPoolInitializedAtZero, accounts, ledger):
    ...
    assert (
        afterTick1.oneMinusPercSwap < beforeTick1.oneMinusPercSwap
    )
...
def test_limitLO_badSwapPrice_zeroForOne(createPoolLow, accounts, ledger):
    ...
    assert pool.ticksLimitTokens0.__contains__(TickMath.MAX_TICK_LO - 5)
    assert (
        pool.ticksLimitTokens0[TickMath.MAX_TICK_LO - 5].oneMinusPercSwap
        == 1
    )
...
def test_limitLO_badSwapPrice_OneForZero(createPoolLow, accounts, ledger):
    ...
    assert pool.ticksLimitTokens1.__contains__(TickMath.MIN_TICK_LO + 5)
    assert (
        pool.ticksLimitTokens1[TickMath.MIN_TICK_LO + 5].oneMinusPercSwap
        == 1
    )
...
#             assert tick.oneMinusPercSwap - percSwappedPrev == 333644089714382253081950632032915180131
#         else:
#             print("tick.oneMinusPercSwap", tick.oneMinusPercSwap)
#             print("diff", tick.oneMinusPercSwap - percSwappedPrev )
#             #assert tick.oneMinusPercSwap - percSwappedPrev == 648480935341976477991928071539899
...
#             assert tick.oneMinusPercSwap - percSwappedPrev == 648480935341976477991928071539899 or tick.oneMinusPercSwap - percSwappedPrev == 648480935341976477991928071539900
#         print(
#             "tick.liquidityLeft",
#             tick.liquidityLeft,
#         )
...
#         perc = Decimal(abs(amount1) / liqLeftBeforeSwap).quantize(Decimal(decimalPrecision), rounding=ROUND_UP, context=Context(prec=contextPrecision))
#         print("perc", perc)
#         print("diff", pool.ticksLimitTokens1[0].oneMinusPercSwap - oneMinusPercSwappedPrev)
...
#         percCalcul = ((oneMinusPercSwappedPrev - pool.ticksLimitTokens1[0].oneMinusPercSwap) / oneMinusPercSwappedPrev).quantize(Decimal(decimalPrecision), rounding=ROUND_UP, context=Context(prec=contextPrecision))
...
def check_limitOrderSwap_oneTick_exactIn(
    ...
    if zeroForOne:
        amountOut = -SqrtPriceMath.calculateAmount1LO(amountRemainingLessFee, priceLO, False)
        ticksLimitTokens = pool.ticksLimitTokens1
    else:
        amountOut = -SqrtPriceMath.calculateAmount0LO(amountRemainingLessFee, priceLO, False)
