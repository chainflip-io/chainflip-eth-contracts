...
from ChainflipPoolSimple import *
import SwapMath
import TickMath
...
def createPool(feeAmount, tickSpacing):
    feeAmount = feeAmount
    pool = ChainflipPoolSimple(TEST_TOKENS[0], TEST_TOKENS[1], feeAmount, tickSpacing)
...
def initializedMediumPool(createPoolMedium, accounts):
    ...
    pool.mintLinearOrder(
        TEST_TOKENS[0], accounts[0], closeAligniniTickiRDown, maxTick, 3161
    )
    pool.mintLinearOrder(
        TEST_TOKENS[1], accounts[0], minTick, closeAligniniTickRUp, 3161
    )
...
def test_trialmint(createPoolMedium, accounts):
    pool, minTick, maxTick, _, _ = createPoolMedium
    pool.initialize(encodePriceSqrt(1, 1))
    pool.mintLinearOrder(TEST_TOKENS[0], accounts[0], minTick, maxTick, 3161)
...
def test_fails_tickLower_ltMinTick(initializedMediumPool, accounts):
    ...
    pool, _, _, _, _, _, _ = initializedMediumPool
    tryExceptHandler(
        pool.mintLinearOrder, "TLM", TEST_TOKENS[0], accounts[0], -887273, 0, 1
    )
...
def test_fails_tickUpper_gtMaxTick(initializedMediumPool, accounts):
    ...
    pool, _, _, _, _, _, _ = initializedMediumPool
    tryExceptHandler(
        pool.mintLinearOrder, "TUM", TEST_TOKENS[0], accounts[0], 0, 887273, 1
    )
...
def test_fails_totalAmountatTick_gtMAX(initializedMediumPool, accounts):
    ...
    pool, minTick, maxTick, _, tickSpacing, _, _ = initializedMediumPool
    pool.mintLinearOrder(
        TEST_TOKENS[0], accounts[0], minTick + tickSpacing, maxTick - tickSpacing, 1000
    )
...
def test_initial_balances(initializedMediumPool):
    print("fails if amount is zero")
    pool, _, _, _, _, _, _ = initializedMediumPool
    # Range Order value: 10003
    assert pool.balances[pool.token0] == 10003
    # Range Order value: 1000
    assert pool.balances[pool.token1] == 1002
...
def test_mint_one_side(initializedMediumPool, accounts):
    ...
    pool.mintLinearOrder(TEST_TOKENS[0], accounts[0], closeIniTickRDown, maxTick, 3161)

    assert pool.balances[pool.token0] == 10003 * 2
    assert pool.balances[pool.token1] == 1002

    pool.mintLinearOrder(TEST_TOKENS[1], accounts[0], minTick, closeIniTickRUp, 3161)
    assert pool.balances[pool.token0] == 10003 * 2
    assert pool.balances[pool.token1] == 1002 * 2
...
def test_transferToken0_only(initializedMediumPool, accounts):
    ...
    amount0 = pool.mintLinearOrder(TEST_TOKENS[0], accounts[0], -22980, 0, 10000)
    assert amount0 != 0
    assert pool.balances[TEST_TOKENS[1]] == amount1Before
    assert pool.balances[pool.token0] == 10003 + 21549
...
def test_maxTick_maxLeverage(initializedMediumPool, accounts):
    ...
    pool.mintLinearOrder(
        TEST_TOKENS[0], accounts[0], maxTick - tickSpacing, maxTick, 2**102
    )
    assert pool.balances[pool.token0] == 10003 + 828011525
    assert pool.balances[pool.token1] == 1002
...
def test_maxTick(initializedMediumPool, accounts):
    ...
    pool.mintLinearOrder(TEST_TOKENS[0], accounts[0], -22980, maxTick, 10000)
    assert pool.balances[pool.token0] == 10003 + 31549
    assert pool.balances[pool.token1] == 1002
...
#     pool, _, _, _, _, _, _ = initializedMediumPool
#     pool.mintLinearOrder(TEST_TOKENS[0], accounts[0], -240, 0, 10000)
#     pool.burnLimitOrder(TEST_TOKENS[0], accounts[0], -240, 0, 10000)
...
def test_addLiquidity_toLiquidityGross(initializedMediumPool, accounts):
    ...
    assert getLinearPositionSimple(pool.positionsZero, accounts[0], -240, 0, True).liquidityRemaining == 100
    assert getLinearPositionSimple(pool.positionsZero, accounts[0], -240, 0, True).liquiditySwapped == 0
    # No liquidityGross === tick doesn't exist
    pool.mintLinearOrder(TEST_TOKENS[0], accounts[0], -240, tickSpacing, 150)
    assert getLinearPositionSimple(pool.positionsZero, accounts[0], -240, tickSpacing, True).liquidityRemaining == 150
    assert getLinearPositionSimple(pool.positionsZero, accounts[0], -240, tickSpacing, True).liquiditySwapped == 0
    # No liquidityGross === tick doesn't exist
    pool.mintLinearOrder(TEST_TOKENS[0], accounts[0], 0, tickSpacing * 2, 60)

    assert getLinearPositionSimple(pool.positionsZero, accounts[0], 0, tickSpacing*2, True).liquidityRemaining == 60
    assert getLinearPositionSimple(pool.positionsZero, accounts[0], 0, tickSpacing*2, True).liquiditySwapped == 0
...
def test_removeLiquidity_fromLiquidityGross(initializedMediumPool, accounts):
    ...
    pool, _, _, _, _, _, _ = initializedMediumPool
    pool.mintLinearOrder(TEST_TOKENS[0], accounts[0], -240, 0, 100)
    pool.mintLinearOrder(TEST_TOKENS[0], accounts[0], -240, 0, 40)
    pool.burnLimitOrder(TEST_TOKENS[0], accounts[0], -240, 0, 90)
    assert getLinearPositionSimple(pool.positionsZero, accounts[0], -240, 0, True).liquidityRemaining == 50
    assert getLinearPositionSimple(pool.positionsZero, accounts[0], -240, 0, True).liquiditySwapped == 0


    pool.mintLinearOrder(TEST_TOKENS[1], accounts[0], -240, 0, 100)
    pool.mintLinearOrder(TEST_TOKENS[1], accounts[0], -240, 0, 40)
    pool.burnLimitOrder(TEST_TOKENS[1], accounts[0], -240, 0, 90)
    assert getLinearPositionSimple(pool.positionsOne, accounts[0], -240, 0, False).liquidityRemaining == 50
    assert getLinearPositionSimple(pool.positionsOne, accounts[0], -240, 0, False).liquiditySwapped == 0
...
def test_clearTickLower_ifLastPositionRemoved(initializedMediumPool, accounts):
    ...
    # tick cleared == not in tick
    # We don't clear a tick if it's burnt - not until collected or fully swapped
    #assert not pool.ticksLinearTokens0.__contains__(-240)


 # NOTE: We don't clear a tick if it's burnt - not until collected or fully swapped. At least nor for now.
...
def test_transferCurrentPriceTokens(initializedMediumPool, accounts):
    ...
    assert amount0 == 317
    assert amount1 == 32
    assert pool.balances[pool.token0] == 10003 + 317
    # Linear original: 1000
    assert pool.balances[pool.token1] == 1002 + 32
...
def test_initializes_lowerTick(initializedMediumPool, accounts):
    ...
    assert getLinearPositionSimple(pool.positionsZero, accounts[0], minTick + tickSpacing, maxTick - tickSpacing, True).liquidityRemaining == 100

    pool.mintLinearOrder(
        TEST_TOKENS[1], accounts[0], minTick + tickSpacing, maxTick - tickSpacing, 100
    )
    assert getLinearPositionSimple(pool.positionsOne, accounts[0], minTick + tickSpacing, maxTick - tickSpacing, False).liquidityRemaining == 100
...
def test_works_minMaxTick(initializedMediumPool, accounts):
    ...
    assert pool.balances[pool.token0] == 10003 + 31644
    assert pool.balances[pool.token1] == 1002
    amount1 = pool.mintLinearOrder(
    ...
    assert amount1 == 3170
    assert pool.balances[pool.token0] == 10003 + 31644
    assert pool.balances[pool.token1] == 1002 + 3170


# # def test_removing_includesCurrentPrice(initializedMediumPool, accounts):
# #     print("removing works")
# #     pool, minTick, maxTick, _, tickSpacing, _, _ = initializedMediumPool
# #     pool.mintLinearOrder(TEST_TOKENS[0],accounts[0], minTick + tickSpacing, maxTick - tickSpacing, 100)
# #     pool.burnLimitOrder(TEST_TOKENS[0],accounts[0], minTick + tickSpacing, maxTick - tickSpacing, 100)
# #     (_, _, _, amount0, amount1) = pool.collect(
# #         accounts[0],
# #         minTick + tickSpacing,
# #         maxTick - tickSpacing,
...
# #     print("removing works")
# #     pool, _, _, _, _, _, _ = initializedMediumPool
# #     pool.mintLinearOrder(TEST_TOKENS[0],accounts[0], -46080, -46020, 10000)
# #     pool.burnLimitOrder(TEST_TOKENS[0],accounts[0], -46080, -46020, 10000)
# #     (_, _, _, amount0, amount1) = pool.collect(
...
#     assert positionLinear0.feeGrowthInsideLastX128 == 102084710076281216349243831104605583
#     assert positionLinear1.feeGrowthInsideLastX128 == 10208471007628121634924383110460558
...
#     pool.burnLimitOrder(TEST_TOKENS[0],accounts[0], minTick, maxTick, 0)  ## poke to update fees
...
#     pool, _, _, _, _, _ = createPoolMedium12TickSpacing
#     pool.initialize(encodePriceSqrt(1, 1))
...
#     tryExceptHandler(pool.mintLinearOrder, "", accounts[0], -6, 0, 1)
#     tryExceptHandler(pool.mintLinearOrder, "", accounts[0], 0, 6, 1)


# def test_mint_multiplesOf12(initializedPoolMedium12TickSpacing, accounts):
#     print("mint can be called for multiples of 12")
#     pool, _, _, _, _ = initializedPoolMedium12TickSpacing
#     pool.mintLinearOrder(TEST_TOKENS[0],accounts[0], 12, 24, 1)
#     pool.mintLinearOrder(TEST_TOKENS[0],accounts[0], -144, -120, 1)
...
#     pool.mintLinearOrder(TEST_TOKENS[0],accounts[0], 120000, 121200, liquidityAmount)
#     swapExact1For0(pool, expandTo18Decimals(1), accounts[0], None)
#     (recipient, tickLower, tickUpper, amount, amount0, amount1) = pool.burnLimitOrder(TEST_TOKENS[0],
...
#     pool, _, _, _, _ = initializedPoolMedium12TickSpacing
#     liquidityAmount = expandTo18Decimals(1) // 4
#     pool.mintLinearOrder(TEST_TOKENS[0],accounts[0], -121200, -120000, liquidityAmount)
#     swapExact0For1(pool, expandTo18Decimals(1), accounts[0], None)
#     (recipient, tickLower, tickUpper, amount, amount0, amount1) = pool.burnLimitOrder(TEST_TOKENS[0],
...
# # Extra tests since there are modifications in the python ChainflipPoolSimple
