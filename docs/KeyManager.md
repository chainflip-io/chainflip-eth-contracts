...
## `consumesKeyNonce(struct IShared.SigData sigData, bytes32 contractMsgHash)`
   ...
   Call consumeKeyNonceWhitelisted
...
## `updateCanConsumeKeyNonce(struct IShared.SigData sigData, address[] currentAddrs, address[] newAddrs)` (external)
 ...
 Replaces all the addresses that can call consumeKeyNonce. Must delist all addresses and then
...
## `_consumeKeyNonce(struct IShared.SigData sigData, bytes32 contractMsgHash)` (internal)
...
## `consumeKeyNonce(struct IShared.SigData sigData, bytes32 contractMsgHash)` (external)
 ...
 Checks that the msg.sender is whitelisted before verifying the signature.
