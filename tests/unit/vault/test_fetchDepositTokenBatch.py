...
def test_fetchDepositTokenBatch(
    cf, token, token2, DepositToken, amounts, swapIDs, tokenBools
):
    ...
    callDataNoSig = cf.vault.fetchDepositTokenBatch.encode_input(
        agg_null_sig(cf.keyManager.address, chain.id), swapIDs, tokens
    )

    # Fetch the deposit
    cf.vault.fetchDepositTokenBatch(
        AGG_SIGNER_1.getSigData(callDataNoSig, cf.keyManager.address), swapIDs, tokens
    )
