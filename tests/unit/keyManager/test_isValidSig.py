...
def test_isValidSig(cf):
    ...
    tx = cf.keyManager.isUpdatedValidSig(sigData, cleanHexStr(sigData[0]), KEYID_TO_NUM[AGG])
...
def test_isValidSig_rev_msgHash(cf):
    ...
    with reverts(REV_MSG_MSGHASH):
        cf.keyManager.isUpdatedValidSig(AGG_SIGNER_1.getSigData(JUNK_HEX_PAD), JUNK_HEX_PAD, KEYID_TO_NUM[AGG])
...
def test_isValidSig_rev_sig(cf):
    ...
    with reverts(REV_MSG_SIG):
        cf.keyManager.isUpdatedValidSig(sigData, cleanHexStr(sigData[0]), KEYID_TO_NUM[AGG])
