...
def test_isValidSig(cf):
    ...
    tx = cf.keyManager.isValidSig(cleanHexStr(sigData[0]), sigData, KEYID_TO_NUM[AGG])
...
def test_isValidSig_rev_msgHash(cf):
    ...
    with reverts(REV_MSG_MSGHASH):
        cf.keyManager.isValidSig(JUNK_HEX, AGG_SIGNER_1.getSigData(JUNK_HEX), KEYID_TO_NUM[AGG])
...
def test_isValidSig_rev_sig(cf):
    ...
    with reverts(REV_MSG_SIG):
        cf.keyManager.isValidSig(cleanHexStr(sigData[0]), sigData, KEYID_TO_NUM[AGG])
