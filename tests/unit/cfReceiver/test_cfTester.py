...
def test_cfReceive_gas(
    cf,
    cfTester,
):
    gas_message = cfTester.GAS_MESSAGE()

    cf.ALICE.transfer(cf.vault.address, TEST_AMNT * 10)
    tx = cfTester.cfReceive(0, 0, gas_message, NON_ZERO_ADDR, 0, {"from": cf.vault})
    assert tx.gas_used > 6.5 * 10**6
    assert "ReceivedxSwapAndCall" in tx.events

    tx = cfTester.cfReceivexCall(0, 0, gas_message, {"from": cf.vault})
    assert tx.gas_used > 6.5 * 10**6
