...
#     tx = cfDeploy.stakeManager.stake(JUNK_HEX, MIN_STAKE, NON_ZERO_ADDR, {'from': a[0]})
#     print(f'Stake: {tx.gas_used}')


# def test_executeClaim(a, cfDeploy):
#     args = (JUNK_HEX, TEST_AMNT, a[0], chain.time() + CLAIM_DELAY + 10)
...
#     tx = cfDeploy.stakeManager.executeClaim(JUNK_HEX, {'from': a[0]})
#     print(f'executeClaim: {tx.gas_used}')


# def test_setEmissionPerBlock(a, cfDeploy):
#     callDataNoSig = cfDeploy.stakeManager.setEmissionPerBlock.encode_input(agg_null_sig(), JUNK_HEX)
#     tx = cfDeploy.stakeManager.setEmissionPerBlock(GOV_SIGNER_1.getSigData(callDataNoSig), JUNK_HEX, {'from': a[0]})
