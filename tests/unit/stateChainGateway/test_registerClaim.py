from consts import *
from shared_tests import *
from brownie import reverts, web3, chain
from brownie.test import given, strategy


@given(
    st_amount=strategy("uint", exclude=0),
    st_funder=strategy("address"),
    # 5s because without a buffer time, the time that the tx actually executes may have changed
    # and make the test fail even though nothing is wrong
    st_expiryTimeDiff=strategy("uint", min_value=5, max_value=365 * DAY),
)
def test_registerClaim_st_amount_rand(cf, st_amount, st_funder, st_expiryTimeDiff):
    args = (
        JUNK_HEX,
        st_amount,
        st_funder,
        getChainTime() + CLAIM_DELAY + st_expiryTimeDiff,
    )

    if st_amount == 0:
        with reverts(REV_MSG_NZ_UINT):
            signed_call_cf(cf, cf.stateChainGateway.registerClaim, *args)
    else:
        registerClaimTest(
            cf,
            cf.stateChainGateway,
            JUNK_HEX,
            MIN_FUNDING,
            st_amount,
            st_funder,
            getChainTime() + (2 * CLAIM_DELAY),
        )


# Specific st_amounts that should/shouldn't work


def test_registerClaim_min_expiryTime(cf):
    registerClaimTest(
        cf,
        cf.stateChainGateway,
        JUNK_HEX,
        MIN_FUNDING,
        MIN_FUNDING,
        cf.DENICE,
        getChainTime() + CLAIM_DELAY + 5,
    )


def test_registerClaim_rev_just_under_min_expiryTime(cf, fundedMin):
    _, st_amount = fundedMin

    args = (JUNK_HEX, st_amount, cf.DENICE, getChainTime() + CLAIM_DELAY - 5)

    with reverts(REV_MSG_EXPIRY_TOO_SOON):
        signed_call_cf(cf, cf.stateChainGateway.registerClaim, *args)


def test_registerClaim_claim_expired(cf, fundedMin):
    _, st_amount = fundedMin
    args = (JUNK_HEX, st_amount, cf.DENICE, getChainTime() + CLAIM_DELAY + 5)

    signed_call_cf(cf, cf.stateChainGateway.registerClaim, *args)

    chain.sleep(CLAIM_DELAY + 10)
    registerClaimTest(
        cf,
        cf.stateChainGateway,
        JUNK_HEX,
        MIN_FUNDING,
        MIN_FUNDING,
        cf.DENICE,
        getChainTime() + (2 * CLAIM_DELAY),
    )


def test_registerClaim_rev_claim_not_expired(cf, fundedMin):
    _, st_amount = fundedMin
    args = (JUNK_HEX, st_amount, cf.DENICE, getChainTime() + CLAIM_DELAY + 5)

    signed_call_cf(cf, cf.stateChainGateway.registerClaim, *args)

    with reverts(REV_MSG_CLAIM_EXISTS):
        signed_call_cf(cf, cf.stateChainGateway.registerClaim, *args)


def test_registerClaim_rev_nodeID(cf, fundedMin):
    _, st_amount = fundedMin
    args = (0, st_amount, cf.DENICE, getChainTime() + CLAIM_DELAY + 5)

    with reverts(REV_MSG_NZ_BYTES32):
        signed_call_cf(cf, cf.stateChainGateway.registerClaim, *args)


def test_registerClaim_rev_st_funder(cf, fundedMin):
    _, st_amount = fundedMin

    with reverts(REV_MSG_NZ_ADDR):
        args = (JUNK_HEX, st_amount, ZERO_ADDR, getChainTime() + CLAIM_DELAY + 5)
        signed_call_cf(cf, cf.stateChainGateway.registerClaim, *args)


def test_registerClaim_rev_sig(cf, fundedMin):
    _, st_amount = fundedMin
    args = (JUNK_HEX, st_amount, cf.DENICE, getChainTime() + CLAIM_DELAY + 5)

    sigData = AGG_SIGNER_1.getSigDataWithNonces(
        cf.keyManager, cf.stateChainGateway.registerClaim, nonces, *args
    )

    sigData_modif = sigData[:]
    sigData_modif[0] += 1
    with reverts(REV_MSG_SIG):
        cf.stateChainGateway.registerClaim(sigData_modif, *args, {"from": cf.ALICE})

    sigData_modif = sigData[:]
    sigData_modif[1] += 1
    with reverts(REV_MSG_SIG):
        cf.stateChainGateway.registerClaim(sigData_modif, *args, {"from": cf.ALICE})

    sigData_modif = sigData[:]
    sigData_modif[2] = NON_ZERO_ADDR
    with reverts(REV_MSG_SIG):
        cf.stateChainGateway.registerClaim(sigData_modif, *args, {"from": cf.ALICE})


@given(
    st_sender=strategy("address"),
)
def test_registerClaim_rev_suspended(cf, fundedMin, st_sender):
    _, st_amount = fundedMin

    cf.stateChainGateway.suspend({"from": cf.GOVERNOR})

    with reverts(REV_MSG_GOV_SUSPENDED):
        args = (JUNK_HEX, st_amount, cf.DENICE, getChainTime() + CLAIM_DELAY)
        signed_call_cf(cf, cf.stateChainGateway.registerClaim, *args, sender=st_sender)
