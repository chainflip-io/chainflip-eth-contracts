...
def test_fetchDepositEth_transfer_fetchDepositToken_transfer(cf, token, DepositEth, DepositToken):
    ...
    with reverts():
        cf.vault.transfer(AGG_SIGNER_1.getSigData(callDataNoSig, cf.keyManager.address), ETH_ADDR, cf.BOB, TEST_AMNT)
    ...
    callDataNoSig = cf.vault.transfer.encode_input(agg_null_sig(cf.keyManager.address, chain.id), token, cf.BOB, amount)
    cf.vault.transfer(AGG_SIGNER_1.getSigData(callDataNoSig, cf.keyManager.address), token, cf.BOB, amount)
...
def test_fetchDepositTokenBatch_transferBatch_fetchDepositEthBatch_transferBatch(cf, token, DepositEth, DepositToken):
    ...
    callDataNoSig = cf.vault.transferBatch.encode_input(agg_null_sig(cf.keyManager.address, chain.id), [token, token], [cf.ALICE, cf.BOB], [amountAlice, amountBob])
    balanceBefore = cf.CHARLIE.balance()
    ...
    with reverts(REV_MSG_ERC20_EXCEED_BAL):
        cf.vault.transferBatch(AGG_SIGNER_1.getSigData(callDataNoSig, cf.keyManager.address), [token, token], [cf.ALICE, cf.BOB], [amountAlice, amountBob])
