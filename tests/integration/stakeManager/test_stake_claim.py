...
def test_registerClaim_stake_executeClaim_stake_registerClaim_executeClaim(cf):
    ...
    # 1st stake
    cf.flip.approve(cf.stakeManager.address, stakeAmount1, {"from": cf.ALICE})
    stakeTx1 = cf.stakeManager.stake(
        nodeID1, stakeAmount1, NON_ZERO_ADDR, {"from": cf.ALICE}
    )
    ...
    # 2nd stake
    cf.flip.approve(cf.stakeManager.address, stakeAmount2, {"from": cf.BOB})
    stakeTx2 = cf.stakeManager.stake(
        nodeID2, stakeAmount2, NON_ZERO_ADDR, {"from": cf.BOB}
    )
